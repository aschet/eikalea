...
def resolve_gpt2_nudge_template_path(args: argparse.Namespace) -> str:
    """--gpt2-nudge-template, if given, overrides the packaged default
    (GPT2_NUDGE_TEMPLATE_PATH, the medium/palette/mood line). --gpt2-mode
    seed always resolves a nudge template now -- confirmed empirically
    that a bare/empty seed_text gives GPT-2 far less varied drafts than a
    real one: 19 duplicate drafts out of 100 consecutive seeds with
    nothing given, vs 0 with a real nudge (see gpt2_expander.py's
    generate_gpt2_draft). Pure free-association is still reachable by
    pointing this at an empty file: the resolved nudge text would then be
    empty, and generate_gpt2_draft's own retry-on-empty path takes over
    exactly as if no nudge had been given at all."""
...
def next_user_message(seed: int, args: argparse.Namespace, axis_gen: Iterator[str] | None) -> str | None:
    ...
    continuation seed (see generate_gpt2_nudged_draft), then through the
    separate gpt2-seed template (see gpt2_expander.py) -- entirely
    bypassing the six-axis template/wildcards for the synthesis pass
    itself. --gpt2-mode subject keeps the six-axis template but swaps in
    a fresh GPT-2 draft for just the subject axis, per seed -- everything
    else still comes from the curated pools. Otherwise, the six-axis path
